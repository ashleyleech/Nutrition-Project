...
namespace Nutrition01
{
    public static class Parms
    {
        ...
        public static bool addNewParm(string inputRecord, int lineNumber)
        {
            ...
            // Add the new entry to the ParmsList
            List<List<double>> temp = new List<List<double>>();
            if (!parseParms(parmName, parmString, temp)) return false;
            parmsList.Add(parmName.ToLower(), temp);

            double foo = getParmDouble(parmName);
        ...
        /// </summary> ////////////////////////////////////////////////////////////////////////
        public static bool parseParms(string parmName, string parmString, List<List<double>> parmList)
        {
            string[] parmsUncoverted = Regex.Split(parmString, parmListDelimiter);

            if (parmsUncoverted.Length < 1)
            {
                Console.WriteLine("No values provided for parameter  \"{0}\".", parmName);
                Console.WriteLine("Hit any key to continue.");
                Console.ReadKey();
                return false;
            }
            ...
            foreach (var ntuppleString in parmsUncoverted)
            {
                ...
                if (ntuppleElements.Length < 1)
                {
                    Console.WriteLine("Entry in parameter list is empty.  See parameter \"{0}\".", parmName);
                    Console.WriteLine("Hit any key to continue.");
                    Console.ReadKey();
                    return false;
                }
                ...
                {
                    double elementValue;
                    if (!Double.TryParse(element,out elementValue))
                    {
                        Console.WriteLine("Problem in parameter file for parameter \"{0}\".", parmName);
                        Console.WriteLine("Cannot convert \"{0}\" to double.", element);
                        Console.WriteLine("Hit any key to continue.");
                        Console.ReadKey();
                        return false;
                    }
